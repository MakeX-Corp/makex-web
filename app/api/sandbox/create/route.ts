...
export async function POST(request: Request) {
  ...
  try {
    ...
    console.log("App data retrieved:", { id: appData.id, name: appData.app_name, sandbox_status: appData.sandbox_status });
    ...
    while (!isReady && attempts < maxAttempts) {
      try {
        console.log(`Attempt ${attempts + 1}/${maxAttempts} to check app readiness`);
    ...
    if (!isReady) {
      console.log("Timed out waiting for app URL to be ready after", maxAttempts, "attempts");
    }
    
    console.log("Returning success response with URLs:", { app_url: appUrl, api_url: apiUrl });
    return NextResponse.json({
      success: true,
