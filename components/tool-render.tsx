...
export default function ToolInvocation({ part }: { part: any }) {
    ...
    // Handle new tool format (type: 'tool-readFile', input/output structure)
    const isNewFormat = part.type && part.type.startsWith('tool-');
    ...
    if (isNewFormat) {
      ...
      const isInProgress = part.state === "partial-call" || part.state === "call";
                        ...
                        {showCodePopup && (
                          <div className="fixed inset-0 bg-background/80 backdrop-blur-sm z-50 flex items-center justify-center" onClick={() => setShowCodePopup(false)}>
                            <div className="bg-card p-4 rounded-lg shadow-lg w-3/4 max-h-[80vh] overflow-auto" onClick={(e) => e.stopPropagation()}>
                              ...
                              <CodeRenderer 
                                content={
                                  toolName === 'readFile' ? part.output.data : 
                                  part.input.content
                                }
                                language="javascript"
                              />
    ...
    // Handle old tool format (backward compatibility)
