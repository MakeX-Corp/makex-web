  ...
  run: async (payload: {
    ...
    try {
      ...
      const { error: startingError } = await adminSupabase
        ...
        .eq("id", sandboxId);
      if (startingError) {
        console.error("[startExpo] Error setting expo_status to 'starting':", startingError);
        throw new Error(`Failed setting expo_status to 'starting': ${startingError.message}`);
      }
      ...
      const { error: bundlingError } = await adminSupabase
        ...
        .eq("id", sandboxId);
      if (bundlingError) {
        console.error("[startExpo] Error setting expo_status to 'bundling':", bundlingError);
        throw new Error(`Failed setting expo_status to 'bundling': ${bundlingError.message}`);
      }
      ...
      while (attempt < maxAttempts) {
        attempt++;
        try {
          console.log(`[startExpo] Checking app preview availability, attempt ${attempt}`);
          appPreviewResponse = await fetch(appUrl);
          if (appPreviewResponse.ok) {
            console.log("[startExpo] App preview is available.");
            break;
          } else {
            console.log(`[startExpo] App preview not ready, status: ${appPreviewResponse.status}`);
          }
        } catch (err) {
      ...
      if (!appPreviewResponse || !appPreviewResponse.ok) {
        throw new Error("App preview never became available after multiple attempts.");
      }
      await redisUrlSetter(appName, appUrl, apiUrl);

      // Set status to 'ready' at the end
      console.log("[startExpo] Updating sandbox with URLs and setting status to ready");
